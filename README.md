...
dataProfilerR is on [CRAN](https://CRAN.R-project.org/package=dataProfilerR):
...
- `statistics` — numeric summary, categorical summary, correlation matrices, and
  the categorical association matrix.
- `diagnostics` — missingness, normality, outliers, date-column profile, the
  grouped comparison (when `group_by` is set), and the quality score.
- `plots` — the `ggplot2` objects (empty if `build_plots = FALSE`; the per-column
  distribution plots are also skipped when `distributions = FALSE`).
...
- **Numeric-vs-categorical effect sizes** (e.g. eta-squared, group-mean
  differences with tests) aren't here yet; `compare_groups()` reports descriptive
  summaries only, not significance.
...
- **Distribution plots are still eager unless you opt out** with
  `distributions = FALSE`; a fully lazy, build-on-demand path would be cleaner.
- **`report()` requires pandoc** (the usual R Markdown dependency); there is no
